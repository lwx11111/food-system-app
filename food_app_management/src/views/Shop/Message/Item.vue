                ...
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item
                                ...
                                prop="receiveUserId">
                            <el-input
                                    v-model="data.item.receiveUserId"
                                    :disabled="data.disabled">
                            </el-input>
                        </el-form-item>
                    </el-col>
...
<script lang="ts" setup>
    ...
    const init = (id, type) => {
        ...
        if (data.type === 'detail' || data.type === 'update') {
            ...
            // 发送请求
            Api.sel4message(data.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    data.item = res.data;
                    // 界面显示
    ...
    const saveOrUpdate = () => {
        // 保存或更新操作
        if (data.type === 'update') {
            Api.update4message(data.id, data.item).then(res => {
                if (res.code === 200){
                    ElMessage({
                      message: '修改成功',
                      type: 'success',
            ...
            Api.add4message(data.item).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                      message: '保存成功',
