            ...
            <div style="display: flex;"
                ...
                <el-form :inline="true"
                    ...
                    <el-form-item label="发送方id">
                        <el-input placeholder="请输入发送方id"
                                  v-model="data.formList.sendUserId"
                                  style="width: 200px"
                                  @keyup.enter.native="getData">
                        </el-input>
                    </el-form-item>
                    <el-form-item label="接收方id">
                        <el-input placeholder="请输入接收方id"
                                  v-model="data.formList.receiveUserId"
                                  style="width: 200px"
                                  @keyup.enter.native="getData">
                        </el-input>
                    </el-form-item>
...
<script lang="ts" setup>
    ...
    const data = reactive({
        ...
        detailUrl: '/name/message/item', // detailUrl:详情页面地址
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            sendUserId : data.formList.sendUserId,
            receiveUserId : data.formList.receiveUserId,
            content : data.formList.content,
            sendTime : data.formList.sendTime,
            parentId : data.formList.parentId,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4message(params).then(res=> {
            if (res.code === 200){
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/message/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4message(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4message(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4message(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
