                ...
                <el-collapse-item name="1">
                    ...
                    <div style="display: flex;"
                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="只有一家店铺">
                                <el-input placeholder="请输入只有一家店铺"
...
<script lang="ts" setup>
    import Api from '@/api/api_shopitemcategory.js'
    ...
    const data = reactive({
        ...
        detailUrl: '/name/shopitemcategory/item', // detailUrl:详情页面地址
    ...
    const getData = () => {
        ...
        // 后台请求
        Api.selpage4shopitemcategory(params).then(res=> {
            if (res.code === 200){
    ...
    const downloadExcelTemplate = () => {
        ...
        Api.downloadExcelTemplate(params).then(data => {
            ...
            a.href = blobUrl
            a.download = 'ShopItemCategory.xls'
            a.click()
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/shopitemcategory/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4shopitemcategory(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4shopitemcategory(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4shopitemcategory(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
