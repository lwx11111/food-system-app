...
<script lang="ts" setup>
    ...
    const data = reactive({
        operateTitle: '新增',
        backUrl: '/name/menucollection/index',
        type: '',
        showBtn: true,
        disabled: false,
        id: 0,
    ...
    const init = (id, type) => {
        ...
        if (data.type === 'detail' || data.type === 'update') {
            ...
            // 发送请求
            Api.sel4menucollection(data.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    data.item = res.data;
                    // 界面显示
    ...
    const saveOrUpdate = () => {
        // 保存或更新操作
        if (data.type === 'update') {
            Api.update4menucollection(data.id, data.item).then(res => {
                if (res.code === 200){
                    ElMessage({
                      message: '修改成功',
                      type: 'success',
            ...
            Api.add4menucollection(data.item).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                      message: '保存成功',
