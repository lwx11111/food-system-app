...
<script lang="ts" setup>
    ...
    import CategoryApi from '@/api/Community/api_category.js';
    ...
    const data = reactive({
        ...
        detailUrl: '/name/community/item', // detailUrl:详情页面地址
    ...
    const categoryFormat = (row, column) => {
        for (let i = 0; i < data.category.length; i++) {
            if (row.categoryId === data.category[i].id) {
    ...
    const getCategory = () => {
        const data1 = {};
        CategoryApi.selpage4category(data1).then(res => {
            console.log(res)
            ...
            data.category = res.data.records;
        })
    }
    const getData = () => {
        // 查询方法
        ...
        // 后台请求
        Api.selpage4community(params).then(res=> {
            if (res.code === 200){
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/community/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4community(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4community(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4community(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
