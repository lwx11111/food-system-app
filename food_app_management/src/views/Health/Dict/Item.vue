                ...
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="类型" prop="value">
                            <el-select v-model="data.item.value" :disabled="data.disabled">
                                <el-option value="0" label="蔬菜"></el-option>
                                <el-option value="1" label="荤菜"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                </el-row>
...
<script lang="ts" setup>
    import Api from '@/api/sys/api_sysdict.js'
    ...
    const data = reactive({
        ...
        rules: {
          name: [
              { required: true, message: '食物名不能为空', trigger: 'blur' }
          ],
          value: [
    ...
    const init = (id, type) => {
        ...
        if (data.type === 'detail' || data.type === 'update') {
            ...
            // 发送请求
            Api.sel4sysdict(data.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    data.item = res.data;
                    // 界面显示
    ...
    const saveOrUpdate = () => {
        // 保存或更新操作
        if (data.type === 'update') {
            Api.update4sysdict(data.id, data.item).then(res => {
                if (res.code === 200){
                    ElMessage({
                      message: '修改成功',
                      type: 'success',
            ...
            Api.add4sysdict(data.item).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                      message: '保存成功',
