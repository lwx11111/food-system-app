...
<script lang="ts" setup>
    import Api from '@/api/Menu/api_menucollection.js'
    ...
    const data = reactive({
        ...
        detailUrl: '/name/menucollection/item', // detailUrl:详情页面地址
    ...
    const getData = () => {
        ...
        // 后台请求
        Api.selpage4menucollection(params).then(res=> {
            if (res.code === 200){
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/menucollection/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4menucollection(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4menucollection(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4menucollection(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
